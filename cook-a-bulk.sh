...
start_to_cook () {
    ...
    questione_Host () {
        while true; do
            read -p "Do you wish to change host addr? (default: ${addrHost}) [y/n]: " yn
            case $yn in
                [Yy]* ) return 0; break;;
...
cook_version () {
    ...
    questione_Version () {
        while true; do
        read -p "Do you wish to change the Dgraph Version? (default: ${dgraphVersion}) [y/n]: " yn
        case $yn in
                [Yy]* ) return 0; break;;
...
cook_Bind () {
    ...
    questione_TheBind () {
        while true; do
            read -p "Do you wish to change the bindall default? (default: ${bindall}) [y/n]: " yn
            case $yn in
                [Yy]* ) return 0; break;;
...
cook_Storage () {
    ...
    questione_The_StorageType () {
        while true; do
            read -p "Do you wish to change the Storage type? (default: ${StorageType}) [y/n]: " yn
            case $yn in
                [Yy]* ) return 0; break;;
...
cook_Memory () {
    ...
    questione_Memory () {
        while true; do
            read -p "Do you wanna increase the memory? (default: ${my_alpha_memory}) [y/n]: " yn
            case $yn in
                [Yy]* ) return 0; break;;
    ...
    if questione_Memory; then
        input_variableM=$my_alpha_memory
...
generate_docker_compose () {
    ...
    render_template ./templates/docker-compose.yaml > ./docker-compose.yaml
}
